...
- Create a branch using the following naming convention in the terminal.
  ...
  *Note*: The [pair-branch](https://github.com/Devbootcamp/pair-branch) utility can be installed from DBC.
  ...
  ```
- **Make sure at least your first and final commits have the following keywords**. We use these to keep track of completion automatically. You can still add your message, but the message must contain the keywords with colons on their own.

  First commit when starting a challenge
  ```
  ":started"
  # example command
  $ git commit -m "initial commit, starting the challenge :started"
  ```
  Any commit where you feel stuck or couldn't get through it:
  ```
  ":blocked"
  # example command
  $ git commit -m "couldn't figure out the challenge at this point :blocked"
  ```
  Final commit when finishing a challenge and before submitting a pull request:
  ```
  ":completed"
  # example command
  $ git commit -m "finished the challenge :completed"
  ```
